...
Одним из решений этой проблемы стало использование априорной информации относительно распределения параметров и ковариационной матрицы ошибок, т. е. переход от обычных\footnote{В англоязычной литературе обычные VAR (без наложения априорных распределений) называются частотными "--- \begin{otherlanguage}{english}frequentist.\end{otherlanguage}} VAR к Байесовским (Bayesian VAR, BVAR).
...
Однако \cite{koop_korobilis_2010} не рассматривают ставший весьма популярным метод задания априорного распределения через добавление дополнительных наблюдений, в т. ч. априорное распределение суммы коэффициентов (\begin{otherlanguage}{english}sum-of-coefficients prior\end{otherlanguage}) и априорное распределение начального наблюдения (\begin{otherlanguage}{english}initial observation prior\end{otherlanguage}).
...
\item Независимое нормальное-обратное Уишарта априорное распределение (\begin{otherlanguage}{english}Independent Normal-inverse Wishart prior\end{otherlanguage})
...
\item Независимое нормальное-Джеффриса априорное распределение (\begin{otherlanguage}{english}Independent normal Jeffreys prior\end{otherlanguage})
...
\item Неинформативное-Джеффриса априорное распределение (\begin{otherlanguage}{english}Diffuse Jeffreys prior\end{otherlanguage})
...
\item Сопряженное нормальное-обратное Уишарта распределение (\begin{otherlanguage}{english}Conjugate normal-inverse Wishart prior\end{otherlanguage})
...
\item Сопряжённое нормальное-Джеффриса априорное распределение (\begin{otherlanguage}{english}Conjugate normal Jeffreys prior\end{otherlanguage})
...
\item Неинформативное-Джеффриса (\begin{otherlanguage}{english}Diffuse Jeffreys prior\end{otherlanguage})
...
Предположим, прогноз на момент $\tau$ имеет \textit{ex ante}  функцию распределения\footnote{Апостериорная прогнозная функция распределения получается взятием интеграла от апостериорной прогнозной функции плотности.} $F$ и фактическое значение прогнозируемой переменной \begin{otherlanguage}{english}\textit{ex post}\end{otherlanguage}  равно $y_{\tau}$. Тогда если фактические реализации действительно представляют собой выборку из указанного распределения, тогда значения PIT, определенные как $F(y_{\tau})$, имеют \textit{ex ante} равномерное распределение на промежутке $[0, 1]$. Оценка качества прогноза на основе PIT происходит с помощью гистограммы, вид которой должен напоминать функцию плотности равномерного распределения (с точностью до константы).

Строго говоря, в отличие от анализа функции плотности прогноза (\begin{otherlanguage}{english}predictive density\end{otherlanguage}), метод  PIT является частотным, т.к. сравнивает функцию данных с \textit{ex ante} распределением, которое эта функция имела бы, если бы процесс генерации данных совпадал с тем, что используется моделью. Некоторые формальные тесты на основе PIT можно найти в \cite{geweke_amisano_2010}.

Можно выделить две проблемы, связанные с применением PIT. Во-первых, на практике условие равномерности распределения PIT сложно проверить: даже если рассматриваемая модель действительно является процессом генерации данных для некоторой переменной, прогнозная плотность не обязательно будет с ней совпадать из-за неопределенности относительно параметров. Во-вторых,  в работе \cite{gneiting_al_2007} c помощью симуляций было показано, что на основе PIT невозможно однозначно выбрать истинную модель среди нескольких конкурирующих. Поэтому в этой же работе было предложено максимизировать «остроту прогнозной плотности при заданной калибровке», где под остротой понимается концентрация распределений вокруг фактических реализаций, что, в свою очередь, можно оценить с помощью диаграмм размаха (\begin{otherlanguage}{english}box plots\end{otherlanguage}) или скоринговых правил.
