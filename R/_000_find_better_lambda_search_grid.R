...
# select bad models 
set.seed(777)
fits_bad <- fits_long_lasso %>% filter(corner_optim == "TRUE")
fits_bad_sample <- sample_n(fits_bad, size = 5)
fits_bad_sample$gran_1 <- 1000
fits_bad_sample$gran_2 <- 30
fits_bad_sample <- dplyr::select(fits_bad_sample, -corner_optim)

n_cores <- 1
fits_bad_sample_info <- forecast_all(fits_bad_sample, var_sets, basefolder = basefolder,
                            n_cores = n_cores)


fits_bad_sample2 <- mutate(fits_bad_sample, 
    corner_optim = get_optimum_at_border_flags(model_filename, basefolder)) 
