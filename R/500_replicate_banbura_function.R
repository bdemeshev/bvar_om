...
# use wrong formulas from carriero code for dummy cNIW without square root for
# sigma^2
carriero_hack <- FALSE
...
actual_obs <- melt(df, id.vars = "t") %>% rename(actual = value) %>% mutate(variable = as.character(variable))
