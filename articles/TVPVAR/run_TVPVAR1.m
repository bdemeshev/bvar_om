...
% if you set a seed for both codes. It is part of a package that comes with
% some notes and is available under www.bkolb.eu/codes/TVPVAR.zip.
% This version of the code has no burn-in and the order of draws in the
% Gibbs sampler is different to the one in my note (it has to be the same
% as in Del Negro and Primiceri if the results are to be replicated exactly
% using the seed). For further information, see my note and the mentioned
% papers.
%
...
%%%%%%%%%%%%%%%%%%%%
% For comparison to Del Negro and Primiceri (2015): set seed
rng(42);
disp(rand(1))
%%%%%%%%%%%%%%%%%%%%
...
nrep = 50;      % # iterations for Gibbs sampler (Primiceri: 110000)
burn = 10;      % burn-in period                 (Primiceri: 109000)
...
S2_s = zeros(1,1,nrep+1);
S3_s = zeros(2,2,nrep+1);
...
mm   = 2;    % counter for iteration of Gibbs sampler w/o burn-in
...
for m = 1:nrep
    ...
    [B, resB] = KalmanCarterKohn(Y, X, reshape(Bpr,nB,1), VBpr, nlag, ...
        Omega, Q, 0);
    ...
    % recursion
    [A, resA] = KalmanCarterKohn(resB(:,2:3), XA, Apr, 4*VApr, 1, ...
        Sig, S,0);
    % for debugging/testing:
    ...
    for i=1:ny
        ...
        q = repmat(qj,T,1).*normpdf( repmat(yss(:,i),1,7), ...
            2*repmat(H(:,i),1,7) + repmat(mj,T,1), ...
            repmat(sqrt(v2j),T,1)  );
