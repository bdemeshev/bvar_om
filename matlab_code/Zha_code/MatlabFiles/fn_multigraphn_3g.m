function scaleout = fn_multigraphn_3g(imfn,...
...
if (nrowg*ncolg < nrow*ncol)
   ...
   error('fn_multigraphn_3g.m: nrowg*ncolg must be greater than nrow*ncol')
end
...
for i = 1:nrow      % column: from top to bottom
   ...
   for j = 1:ncol        % row: from left to right
      ...
      plot(t,nseries(:,[1 3]), '--m','LineWidth',0.9); %,'Color','k');
      hold on
      plot(t,nseries(:,[2]), ':*b','LineWidth',0.8);
