function [Q, indx_fail] = ftd_SRestrictRWZalg_FP_Model12(A0hatinv,Bhat,nvar,nendo,lags,irs,ndraws_terminate)
...
%   nvar = number of total variables.  
%   nendo = number of endogenous variables. 
%   lags = lag length.
...
while (control==0)
   ...
   newmatrix=normrnd(0,1,nendo,nendo);
   [Qendo,Rendo]=qr(newmatrix);
   for i=1:nendo
      if Rendo(i,i)<0
          Qendo(:,i)=-Qendo(:,i);
      end
   end
   Q(1:nendo,1:nendo) = Qendo;
   ...
   if (indx_both_g_gdp)
      ...
      amax = max([max(imf3hat(1:irs(1),1,whichshock) > 0)]);
   end   
   ...
   whichshock = 2;
   %--- gov_tax < 0 and gdp > 0 for irs(1) periods.
   indx_t_gdp = 0;
   if (indx_both_t_gdp)
      amax = max([max(imf3hat(1:irs(1),2,whichshock) < 0); max(imf3hat(1:irs(1),3,whichshock) > 0)]);  %Mountford and Uhlig's article.
   else
      amax = max([max(imf3hat(1:irs(1),2,whichshock) < 0)]);  %Mountford and Uhlig's article.   
   end   
   if (amax==0)  %The restrictions are satisfied.  
      %--- Swiching the sign of the shock and normalize.
      Q(whichshock,:) = -Q(whichshock,:); 
      ...
      if (indx_both_t_gdp)    
         ...
         amin = min([min(imf3hat(1:irs(1),2,whichshock) < 0)]);  %Mountford and Uhlig's article.   
      end
      if (amin==0)       
         continue;   %The restrictions are not satisfied.  Go to the beginning to redraw.
